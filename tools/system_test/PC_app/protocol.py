"""
Greenhouse serial protocol.

Frame: SOF | TYPE | ID | LEN_H | LEN_L | PAYLOAD
ID = (GID << 4) | MID
"""
...
def split_id(id_byte: int):
    return (id_byte >> 4) & 0x0F, id_byte & 0x0F


def build_frame(gid: int, mid: int, payload: bytes = b"") -> bytes:
    length = len(payload)
    return bytes([
        FRAME_SOF,
        TYPE_CMD,
...
def build_set_mode(mode: int):
    return build_frame(GID_SYSTEM, MID_SET_MODE, bytes([mode]))


def build_set_setpoint(setpoint: float):
    return build_frame(GID_SYSTEM, MID_SET_SETPOINT, struct.pack("<f", setpoint))


def build_set_report_mode(report_mode: int):
    return build_frame(GID_SYSTEM, MID_SET_REPORT_MODE, bytes([report_mode]))


def build_request_status():
    return build_frame(GID_SYSTEM, MID_STATUS_REPORT)


def build_manual_actuator(actuator: int, state: int):
    return build_frame(GID_ACTUATOR, MID_MANUAL_ACTUATOR, bytes([actuator, state]))


def build_set_pid(kp: float, ki: float, kd: float):
    return build_frame(GID_PID, MID_SET_PID, struct.pack("<fff", kp, ki, kd))


def build_echo(payload: bytes):
    return build_frame(GID_COMM, MID_ECHO, payload)


def parse_frame(data: bytes):
    if len(data) < FRAME_HEADER_SIZE:
        return None, 0
    if data[0] != FRAME_SOF or data[1] != TYPE_RSP:
        return None, 1
...
def decode_sensor_report(payload):
    if len(payload) < 8:
        return None
    temperature, setpoint = struct.unpack("<ff", payload[:8])
    return {"temperature": temperature, "temp_setpoint": setpoint}


def _u16_or_none(payload, offset):
    return (payload[offset] << 8) | payload[offset + 1] if len(payload) >= offset + 2 else None
...
def decode_status_report(payload):
    ...
    return {
        ...
        "fan_pwm": _u16_or_none(payload, 10),
        "heater_pwm": _u16_or_none(payload, 12),
    }
...
def frame_name(gid, mid):
    gid_name = _GID_NAMES.get(gid, f"GID_0x{gid:X}")
    mid_name = _MID_NAMES.get((gid, mid), f"MID_0x{mid:X}")
    return f"{gid_name}.{mid_name}"


def status_name(gid, mid, status):
    if (name := _STATUS_NAMES.get((gid, mid, status))):
        return name
    return "Command failed" if status == RSP_SYSTEM_FAIL else f"Status 0x{status:02X}"
